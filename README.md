...
- Source state-wise road-type data (national highway vs. state highway vs. other roads), likely by creating a Dataful account or requesting the data directly from MoRTH, and re-run the classification model with this added.
- Extract state-wise time-of-day data from MoRTH's Annexure-42, to see whether the 18:00-21:00 peak holds consistently across states or is concentrated in a few.
- Bring in more recent population estimates (state-level projections beyond Census 2011) once available, to get a more accurate accident rate.
- Extend the city-level analysis (currently limited to accidents and fatalities) with the same collision-type and violation-type breakdowns done at the national level.

## Tech Stack

- Python, pandas, numpy
- plotly, matplotlib (visualization)
- camelot, pdfplumber (PDF table extraction)
- scikit-learn, scipy (modeling and statistical tests)
- Streamlit (dashboard)
...
The `data` folder must be in the same directory as `app.py` for the app to load correctly.
